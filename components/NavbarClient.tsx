...
import { logoutUser } from '@/lib/api/account';
import { UserProfile } from '@/lib/api/account';
...
export function NavbarClient({
  ...
  function handleLanguageSelect(newLang: 'en' | 'id') {
  ...
  const navLinks = [
    { name: translations.store, href: `/${lang}/store` },
    { name: translations.community, href: `/${lang}/community` },
    { name: translations.about, href: `/${lang}/about` },
    { name: translations.support, href: `/${lang}/support` },
  ];
      ...
      <nav className="bg-steam-darkest text-steam-text py-3 md:py-4 px-4 md:px-8 shadow-2xl sticky top-0 z-1000 border-b border-white/5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4 md:gap-8">
            ...
            <Link href={`/${lang}/home`} className="text-xl md:text-2xl font-bold tracking-widest text-white flex items-center shrink-0">
          ...
          <div className="flex items-center gap-2 md:gap-4 text-[10px] md:text-xs">
              ...
              {isLangOpen && (
                <div className="absolute top-full right-0 mt-2 w-40 bg-steam-darkest border border-white/10 shadow-2xl rounded-md py-2 z-210 overflow-hidden">
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-steam-light hover:text-steam-darkest transition-colors uppercase font-bold ${lang === 'en' ? 'text-steam-light' : 'text-inherit'}`}
                  >
                    English
                  </button>
                  <button
                    onClick={() => handleLanguageSelect('id')}
                    className={`w-full text-left px-4 py-3 hover:bg-steam-light hover:text-steam-darkest transition-colors uppercase font-bold ${lang === 'id' ? 'text-steam-light' : 'text-inherit'}`}
                  >
                    Indonesian
                  </button>
                </div>
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-4">
                <Link href={`/${lang}/profile`} className="text-steam-light hover:text-white uppercase transition-colors font-bold px-1">
                  {user?.username}
                </Link>
                <button onClick={handleLogout} className="hover:text-white uppercase transition-colors font-bold px-1 text-[10px] opacity-50">
                  {translations.logout}
                </button>
              </div>
            ) : (
              <Link href={`/${lang}/login`} className="hover:text-white uppercase transition-colors font-bold px-1">{translations.login}</Link>
            )}

            <Link href={`/${lang}/cart`} className="bg-steam-dark text-steam-light hover:bg-steam-dark/80 hover:brightness-125 px-2 md:px-4 py-1.5 rounded-sm transition-all flex items-center gap-2 font-bold active:scale-95">
               <span className="md:hidden text-base">🛒</span>
               <span className="hidden md:inline tracking-widest">{translations.cart}</span>
               {itemCount > 0 && (
